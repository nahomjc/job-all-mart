...
export function FeaturedJobCard({
	...
	return (
		...
		>
			<div
				...
				style={{
					backgroundImage: `
						...
						linear-gradient(315deg, color-mix(in oklch, var(--primary) 4%, white) 25%, var(--card) 25%)
					`,
			...
			<div
				aria-hidden
				className="pointer-events-none absolute inset-0 bg-linear-to-br from-card/95 via-card/70 to-primary/5"
			/>

			<div className="relative flex flex-1 flex-col">
				<div className="flex size-14 shrink-0 items-center justify-center overflow-hidden rounded-full bg-brand-deep text-primary-foreground shadow-lg shadow-brand-deep/30 ring-4 ring-card">
					{job.logoUrl ? (
						<Image
							src={job.logoUrl}
							alt=""
				...
				<div className="mt-8 space-y-3.5">
					<p className="text-sm font-medium text-muted-foreground">
						{job.company}
						{category ? (
							<span className="text-muted-foreground/80"> · {category.name}</span>
						) : null}
					</p>
					<h3 className="text-balance text-[1.55rem] font-extrabold uppercase leading-[1.08] tracking-tight text-foreground sm:text-[1.7rem] xl:text-[1.85rem]">
						{job.title}
					</h3>
					<span className="inline-flex rounded-full bg-brand-deep px-3 py-1 text-[11px] font-semibold tracking-wide text-primary-foreground">
						{employment}
					</span>
				</div>
					...
					<div className="min-w-0">
						...
						{salary ? (
							<p className="mt-1.5 flex items-baseline gap-1">
								<span className="text-2xl font-extrabold tracking-tight text-foreground sm:text-[1.85rem]">
									{salary.amount}
								</span>
